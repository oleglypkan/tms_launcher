...
// Forward declaration (to resolve recursive includes with #include "OptionPage.h")
class COptionPage;
...
public:
    ...
    /// Programmatically shows an item
    void SetActiveItem(COptionItem *pItem);
    ...
    virtual void DoEndDialog(int nRetCode) = 0;
...
protected:
    ...
    virtual bool OnOK(UINT uCode, int nID, HWND hWndCtl);

    /// Called when the \b Apply button is pressed.
    /// Don't forget to call super class implementation when overriding this method.
    virtual bool OnApply(UINT uCode, int nID, HWND hWndCtl);
    ...
    /// Don't forget to call super class implementation when overriding this method.
    virtual bool OnUse(UINT uCode, int nID, HWND hWndCtl);
...
protected:
    ...
    LRESULT OnSelectionChanged(NMHDR* phdr);
...
class COptionSelectionTreeCtrl : public COptionSelectionCtrl, public CWindowImpl<COptionSelectionTreeCtrl, CTreeViewCtrl>
{
public:
    //---------------------------------------------------------------------------------------
...
protected:
    BEGIN_MSG_MAP_EX(COptionSelectionTreeCtrl)
        ...
        REFLECTED_NOTIFY_CODE_HANDLER_EX(TVN_SELCHANGING, OnSelChanging)
        // end of fix
    ...
    // Added by Oleg Lypkan to handle TVN_SELCHANGING message
    LRESULT OnSelChanging(NMHDR* phdr);
    // end of fix
...
class COptionSelectionTabCtrl : public COptionSelectionCtrl, public CWindowImpl<COptionSelectionTabCtrl, CTabCtrl>
{
public:
    //---------------------------------------------------------------------------------------
...
protected:
    BEGIN_MSG_MAP_EX(COptionSelectionTabCtrl)
        ...
        REFLECTED_NOTIFY_CODE_HANDLER_EX(TCN_SELCHANGING, OnSelChanging)
        // end of fix
        REFLECTED_NOTIFY_CODE_HANDLER_EX(TCN_SELCHANGE, OnSelChanged)
        DEFAULT_REFLECTION_HANDLER()
    END_MSG_MAP()
